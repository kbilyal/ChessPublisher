...
def main()->int:
    with tempfile.TemporaryDirectory(prefix='cp-cr-contract-') as td_raw:
        ...
        def transport(op,payload,token):
            ...
            if op=='delete-authorize':return {'ok':True,'key':'12345','verifiedOwner':True,'url':'https://example.invalid/admin','alreadyDeleted':False}
        ...
        if not auth.get('canDelete') or not auth.get('adminUrl'):raise RuntimeError('delete authorize desktop contract mismatch')
        ...
        print(json.dumps({'createProof':True,'publishProofReuse':True,'deleteAuthorize':True,'unlinkProofRemoval':True,'tokenFailClosed':True,'localBridgeCrypto':False,'sourceId':21},indent=2))
